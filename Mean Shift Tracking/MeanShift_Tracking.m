...
%ilk aday-> ilk framede seçilen bölgenin sonraki framede ayný
%koordinatlardaki histogramý
y = y0;
x = x0;
T2 = I2(y:y+H-1,x:x+W-1);%seçilen koordinatlardaki 2.framedeki görüntü parçasý(T2)
p = Density_estim(T2,Lmap,k,H,W,0);%adayýn histogramý
% Number of iterations
step = 1;
% Computation of the similarity value
% between the two PDF.
[fi,w] = Simil_func(q,p,T2,k,H,W);%iki histogram arasýndaki benzerlik
f = cat(2,f,fi);%bulunan deðer benzerlik vektörüne ekleniyor
% Applying Mean-shift algorithm
%yakýnsama koþullarý 1. -> hist. arasý farkýn f_threshden küçük olmasý
%2. -> max_it iterasyon sýnýrýna ulaþmýþ olmak(her framede en fazla kayma itr.)
while f(f_indx)<f_thresh && step<max_it %max_it=5 iken 4 kayma yapabilir.
    ...
    num_y = 0;
    den = 0;
    %new target center'ýn belirlenmesi
    for i=1:H
        for j=1:W
            %her pikselin x ve y yönündeki gauss gradyenleri ile çarpýlýr
    ...
    %gradyenin en yüksek olduðu yere doðru mean shift vektör oluþturulur(dx,dy, x ve y yönündeki bileþenleri)
    %merkezin vektör doðrultusunda güncellenmesi
    if den ~= 0
        dx = round(num_x/den);
        dy = round(num_y/den);
        y = y+dy;
    ...
    if (y<1 || y>height-H) || (x<1 || x>width-W)
        loss = 1;
        Target_Loss_Dialog_Box();%çizilen bölge video görüntüsünden çýktýysa
        uiwait(Target_Loss_Dialog_Box);
        break;
    end
    % 2. iterasyon için Update the target
