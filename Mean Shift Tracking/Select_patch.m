...
% görüntü pencerenin sol alt köþesine hizalansýn
set(gca,'Units','pixels','Position',[1 1 width height])

% Display the image
imshow(I);
rect = getrect;%mouse ile çizilen dikdörtgeni al
rect = floor(rect);
%çizilen dikdörtgenin baþlangýç noktasý (x0,y0) geniþliði(W) ve uzunluðu(H)
x0 = rect(1);
y0 = rect(2);
W = rect(3);
H = rect(4);
T = I(y0:y0+H-1,x0:x0+W-1,:);%dikdörtgen içinde kalan kýsýmdaki görüntü(T)
if graph==1
    figure (3)
