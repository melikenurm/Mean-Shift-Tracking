...
for i=1:H
    for j=1:W
%benzerliði artýrmak için taylor açýlýmýndaki aðýrlýk ifadesinin bulunmasý
%pikselin histogramlardaki bin'lerinin birbirine bölümü
        w(i,j) = sqrt(q(T2(i,j)+1)/p(T2(i,j)+1));%maximizing bhattacharyya coefficient
%pikselin gauss aðýrlýðý ile çarpýlýp tüm pikseller toplanýr
        f = f+w(i,j)*k(i,j);
    end
end
% Normalization of f
